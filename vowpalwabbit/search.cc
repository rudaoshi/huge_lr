...
namespace Search {
    ...
    for (int n=0; n<priv.ec_seq.size(); n++) {  // iterate over every example in the sequence
      ...
      for (size_t n_id=0; n_id < priv.neighbor_features.size(); n_id++) {
        ...
        if (n + offset < 0) // add <s> feature
          ...
          GD::foreach_feature<search_private,add_new_feature>(all.reg.weight_vector, all.reg.weight_mask, other.atomics[ns].begin_, other.atomics[ns].end_, priv, me.ft_offset);
        }
      }
  ...
  void ensure_size(v_array<T>& A, size_t sz) {
    if (A.end_array - A.begin_ < sz) A.resize(sz*2+1, true);
    A.end_ = A.begin_ + sz;
  }
  ...
  template<class T> void push_at(v_array<T>& v, T item, size_t pos) {
    if (v.size() > pos)
      ...
      if (v.end_array > v.begin_ + pos) {
        // there's enough memory, just not enough filler
        v.begin_[pos] = item;
        v.end_ = v.begin_ + pos + 1;
      } else {
  ...
  void generate_training_example(search_private& priv, v_array<float>& losses, bool add_conditioning=true) {
    ...
    if (!priv.is_ldf) {   // not LDF
      ...
      void* labels = allowed_actions_to_ld(priv, priv.learn_ec_ref_cnt, priv.learn_allowed_actions.begin_, priv.learn_allowed_actions.size());
      ...
      ec.ld = labels;
      ec.in_use = true;
      if (add_conditioning) add_example_conditioning(priv, ec, priv.learn_condition_on.begin_, priv.learn_condition_on.size(), priv.learn_condition_on_names.begin_, priv.learn_condition_on_act.begin_);
  ...
  action search_predict(search_private& priv, example* ecs, size_t ec_cnt, ptag mytag, const action* oracle_actions, size_t oracle_actions_cnt, const ptag* condition_on, const char* condition_on_names, const action* allowed_actions, size_t allowed_actions_cnt, size_t learner_id) {
      ...
      if (priv.learn_a_idx >= valid_action_cnt) {
        ...
        if (priv.examples_dont_change)
          ...
          for (size_t i=0; i<ec_cnt; i++) {
            ...
            VW::copy_example_data(priv.all->audit, priv.learn_ec_copy.begin_ +i, ecs+i, label_size, label_copy_fn);
          }

          priv.learn_ec_ref = priv.learn_ec_copy.begin_;
        }
        ...
        if (priv.auto_condition_features) {
          ...
          priv.learn_condition_on.end_ = priv.learn_condition_on.begin_ + condition_on_cnt;   // allow .size() to be used in lieu of _cnt

          memcpy(priv.learn_condition_on.begin_, condition_on, condition_on_cnt * sizeof(ptag));
          ...
          if (condition_on_names == NULL) {
            ...
            strcpy(priv.learn_condition_on_names.begin_, condition_on_names);
          }
        }

        ensure_size(priv.learn_allowed_actions, allowed_actions_cnt);
        memcpy(priv.learn_allowed_actions.begin_, allowed_actions, allowed_actions_cnt*sizeof(action));
    ...
    if ((priv.state == INIT_TRAIN) ||
      ...
      if ((policy >= 0) || gte_here) {
        ...
        if (cached_action_store_or_find(priv, mytag, condition_on, condition_on_names, priv.condition_on_actions.begin_, condition_on_cnt, policy, a, false))
          ...
          if (priv.auto_condition_features)
            for (size_t n=0; n<ec_cnt; n++)
              add_example_conditioning(priv, ecs[n], condition_on, condition_on_cnt, condition_on_names, priv.condition_on_actions.begin_);
          ...
          cached_action_store_or_find(priv, mytag, condition_on, condition_on_names, priv.condition_on_actions.begin_, condition_on_cnt, policy, a, true);
        }
      }
  ...
  void get_training_timesteps(search_private& priv, v_array<size_t>& timesteps) {
    ...
    // finally, if subsample >= 1, then pick (int) that many uniformly at random without replacement; could use an LFSR but why? :P
      ...
      std::sort(timesteps.begin_, timesteps.end_, cmp_size_t);
    }
  }
  ...
  void search_finish(search& sch) {
    ...
    if (! priv.examples_dont_change) {
      ...
      for(example*ec = priv.learn_ec_copy.begin_; ec!=priv.learn_ec_copy.end_; ++ec)
  ...
  void predictor::make_new_pointer(v_array<action>& A, size_t new_size) {
    ...
    action* old_pointer  = A.begin_;
    A.begin_ = (action*)calloc_or_die(new_size, sizeof(action));
    A.end_ = A.begin_ + new_size;
    A.end_array = A.end_;
    memcpy(A.begin_, old_pointer, old_size * sizeof(action));
  }
  ...
  predictor& predictor::add_to(v_array<action>&A, bool& A_is_ptr, action*a, size_t action_count, bool clear_first) {
    ...
    if (old_size > 0) {
      if (A_is_ptr) { // we need to make our own memory
        ...
        make_new_pointer(A, new_size);
        A_is_ptr = false;
        memcpy(A.begin_ + old_size, a, action_count * sizeof(action));
  ...
  predictor& predictor::erase_oracles() { if (oracle_is_pointer) oracle_actions.end_ = oracle_actions.begin_; else oracle_actions.erase(); return *this; }
  ...
  predictor& predictor::add_oracle(v_array<action>& a) { return add_to(oracle_actions, oracle_is_pointer, a.begin_, a.size(), false); }
  ...
  predictor& predictor::set_oracle(v_array<action>& a) { return add_to(oracle_actions, oracle_is_pointer, a.begin_, a.size(), true); }

  predictor& predictor::erase_alloweds() { if (allowed_is_pointer) allowed_actions.end_ = allowed_actions.begin_; else allowed_actions.erase(); return *this; }
  ...
  predictor& predictor::add_allowed(v_array<action>& a) { return add_to(allowed_actions, allowed_is_pointer, a.begin_, a.size(), false); }
  ...
  predictor& predictor::set_allowed(v_array<action>& a) { return add_to(allowed_actions, allowed_is_pointer, a.begin_, a.size(), true); }
  ...
  action predictor::predict() {
    const action* orA = oracle_actions.size() == 0 ? NULL : oracle_actions.begin_;
    const ptag*   cOn = condition_on_names.size() == 0 ? NULL : condition_on_tags.begin_;
    const char*   cNa = NULL;
    if (condition_on_names.size() > 0) {
      condition_on_names.push_back((char)0);  // null terminate
      cNa = condition_on_names.begin_;
    }
    const action* alA = (allowed_actions.size() == 0) ? NULL : allowed_actions.begin_;
