	...
	if (fec->tag_len >0)
	  {
	    ...
	    memcpy(fec->tag,ec->tag.begin_, fec->tag_len);
	  }
...
example *alloc_examples(size_t label_size, size_t count=1)
{
  ...
  for (size_t i=0; i<count; i++) {
    ...
    ec[i].in_use = true;
    ec[i].ft_offset = 0;
    //  std::cerr << "  alloc_example.indices.begin_=" << ec->indices.begin_ << " end=" << ec->indices.end << " // ld = " << ec->ld << "\t|| me = " << ec << std::endl;
  }
  return ec;
}
  ...
  for (size_t j = 0; j < 256; j++)
    {
      ...
      if (ec.audit_features[j].begin_ != ec.audit_features[j].end_array)
        {
          for (audit_data* temp = ec.audit_features[j].begin_;
               temp != ec.audit_features[j].end_; temp++)
            if (temp->alloced) {
              free(temp->space);
              free(temp->feature);
              temp->alloced = false;
