  ...
  float lda_loop(vw& all, v_array<float>& Elogtheta, float* v,weight* weights,example* ec, float power_t)
{
  ...
  for (unsigned char* i = ec->indices.begin_; i != ec->indices.end_; i++)
    num_words += ec->atomics[*i].end_ - ec->atomics[*i].begin_;
  ...
  do
    {
      memcpy(v,new_gamma.begin_,sizeof(float)*all.lda);
      myexpdigammify(all, v);

      memcpy(old_gamma.begin_,new_gamma.begin_,sizeof(float)*all.lda);
      memset(new_gamma.begin_,0,sizeof(float)*all.lda);
      ...
      for (unsigned char* i = ec->indices.begin_; i != ec->indices.end_; i++)
	{
	  feature *f = ec->atomics[*i].begin_;
	  for (; f != ec->atomics[*i].end_; f++)
	    {
  ...
  while (average_diff(all, old_gamma.begin_, new_gamma.begin_) > all.lda_epsilon);

  ec->topic_predictions.erase();
  ec->topic_predictions.resize(all.lda);
  memcpy(ec->topic_predictions.begin_,new_gamma.begin_,all.lda*sizeof(float));

  score += theta_kl(all, Elogtheta, new_gamma.begin_);
...
void save_load(lda& l, io_buf& model_file, bool read, bool text)
{
  ...
  void learn_batch(lda& l)
  {
	...
	float decay = fmin(1.0, exp(l.decay_levels.end_[-2] - l.decay_levels.end_[(int)(-1 - l.example_t+weights_for_w[l.all->lda])]));
	...
	myexpdigammify_2(*l.all, u_for_w, l.digammas.begin_);
      }
...
void end_examples(lda& l)
{
  for (size_t i = 0; i < l.all->length(); i++) {
    ...
    float decay = fmin(1.0, exp(l.decay_levels.last() - l.decay_levels.end_[(int)(-1- l.example_t +weights_for_w[l.all->lda])]));
    for (size_t k = 0; k < l.all->lda; k++) 
      weights_for_w[k] *= decay;
  }
}
