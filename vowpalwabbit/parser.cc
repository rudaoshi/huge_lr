...
uint32_t cache_numbits(io_buf* buf, int filepointer)
{
  ...
  buf->read_file(filepointer,t.begin_,v_length);
  version_struct v_tmp(t.begin_);
  if ( v_tmp != version )
    {
  ...
  if (all.p->write_cache)
    {
	  ...
	  remove(all.p->output->finalname.begin_);
      rename(all.p->output->currentname.begin_, all.p->output->finalname.begin_);
      while(input->files.size() > 0)
	{
      ...
      input->open_file(all.p->output->finalname.begin_, all.stdin_off, io_buf::READ); //pushing is merged into open_file
...
void generateGrams(vw& all, example* &ex) {
  for(unsigned char* index = ex->indices.begin_; index < ex->indices.end_; index++)
    {
...
void setup_example(vw& all, example* ae)
{
  ...
  if (all.ignore_some)
	...
	for (unsigned char* i = ae->indices.begin_; i != ae->indices.end_; i++)
	  if (all.ignore[*i])
	...
	if (all.ignore[*i])
	  {//delete namespace
	    ae->atomics[*i].erase();
	    memmove(i,i+1,(ae->indices.end_ - (i+1))*sizeof(*i));
	    ae->indices.end_--;
	    i--;
	  }
    }
  ...
  if(multiplier != 1) //make room for per-feature information.
    {
      for (unsigned char* i = ae->indices.begin_; i != ae->indices.end_; i++)
	for(feature* j = ae->atomics[*i].begin_; j != ae->atomics[*i].end_; j++)
	  j->weight_index *= multiplier;
      if (all.audit || all.hash_inv)
	for (unsigned char* i = ae->indices.begin_; i != ae->indices.end_; i++)
	  for(audit_data* j = ae->audit_features[*i].begin_; j != ae->audit_features[*i].end_; j++)
	    j->weight_index *= multiplier;
    }
  
  for (unsigned char* i = ae->indices.begin_; i != ae->indices.end_; i++)
    {
      ae->num_features += ae->atomics[*i].end_ - ae->atomics[*i].begin_;
  ...
  if (all.rank == 0) {
    for (vector<string>::iterator i = all.pairs.begin(); i != all.pairs.end();i++)
      {
	ae->num_features 
	  += (ae->atomics[(int)(*i)[0]].end_ - ae->atomics[(int)(*i)[0]].begin_)
	  *(ae->atomics[(int)(*i)[1]].end_ - ae->atomics[(int)(*i)[1]].begin_);
	...
	ae->num_features 
	  += (ae->atomics[(int)(*i)[0]].end_ - ae->atomics[(int)(*i)[0]].begin_)
            *(ae->atomics[(int)(*i)[1]].end_ - ae->atomics[(int)(*i)[1]].begin_)
            *(ae->atomics[(int)(*i)[2]].end_ - ae->atomics[(int)(*i)[2]].begin_);
    ...
    for (vector<string>::iterator i = all.pairs.begin(); i != all.pairs.end();i++)
      {
	ae->num_features
	  += (ae->atomics[(int)(*i)[0]].end_ - ae->atomics[(int)(*i)[0]].begin_) * all.rank;
	ae->num_features
	  += (ae->atomics[(int)(*i)[1]].end_ - ae->atomics[(int)(*i)[1]].begin_) * all.rank;
      }
    for (vector<string>::iterator i = all.triples.begin(); i != all.triples.end();i++)
      {
	ae->num_features
	...
	ae->num_features
	  += (ae->atomics[(int)(*i)[1]].end_ - ae->atomics[(int)(*i)[1]].begin_) * all.rank;
	ae->num_features
	  += (ae->atomics[(int)(*i)[2]].end_ - ae->atomics[(int)(*i)[2]].begin_) * all.rank;
      }
  }
}
}
...
namespace VW{
  ...
  void empty_example(vw& all, example& ec)
  {
	...
	{
	  for (audit_data* temp 
		 = ec.audit_features[*i].begin_;
	       temp != ec.audit_features[*i].end_; temp++)
	    {
	      if (temp->alloced)
		{
		  free(temp->space);
