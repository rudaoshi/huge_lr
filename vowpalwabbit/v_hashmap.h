...
template<class K, class V> class v_hashmap{
 public:
  ...
  size_t base_size() {
    return dat.end_array-dat.begin_;
  }
  ...
  void clear() {
    ...
    memset(dat.begin_, 0, base_size()*sizeof(hash_elem));
    last_position = 0;
    num_occupants = 0;
  }
  ...
  void iter(void (*func)(K,V)) {
    ...
    for (hash_elem* e=dat.begin_; e!=dat.end_array; e++) {
      //hash_elem* e = dat.begin_+lp;
      if (e->occupied) {
  ...
  void double_size() {
    ...
    // re-insert occupants
    for (hash_elem* e=tmp.begin_; e!=tmp.end_; e++) {
      get(e->key, e->hash);
