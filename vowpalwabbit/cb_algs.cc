...
namespace CB_ALGS
{
  ...
  bool know_all_cost_example(CB::label* ld)
  {
    ...
    for (cb_class* cl = ld->costs.begin_; cl != ld->costs.end_; cl++)
      if (cl->cost == FLT_MAX)
        return false;
  ...
  {
    size_t i = 0;
    for (cb_class *cl = ld->costs.begin_; cl != ld->costs.end_; cl ++, i++)
    {
      if( observed_cost(cl) ) {
        return cl;
      }
    ...
    if (!is_test_label(ld))
      {//need to compute exact loss
        ...
        if( know_all_cost_example(ld) ) {
          ...
          for (COST_SENSITIVE::wclass *cl = c.cb_cs_ld.costs.begin_; cl != c.cb_cs_ld.costs.end_; cl ++) {
