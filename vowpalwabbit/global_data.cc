...
{
  if (tag.begin_ != tag.end_){
    ss << ' ';
    ss.write(tag.begin_, sizeof(char)*tag.size());
  }
  return tag.begin_ != tag.end_;
}
...
vw::vw()
{
  ...
  final_prediction_sink.begin_ = final_prediction_sink.end_ =final_prediction_sink.end_array = NULL;
  raw_prediction = -1;
  print = print_result;
