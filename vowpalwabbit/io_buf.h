...
class io_buf {
 public:
  v_array<char> space; //space.begin_ = beginning of loaded values.  space.end = end of read or written values.
  v_array<int> files;
  ...
  size_t fill(int f) {
    if (space.end_array - endloaded == 0)
      {
	size_t offset = endloaded - space.begin_;
	space.resize(2 * (space.end_array - space.begin_));
	endloaded = space.begin_ +offset;
      }
  ...
  virtual void flush() {
	  if (write_file(files[0], space.begin_, space.size()) != (int) space.size())
