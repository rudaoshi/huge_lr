...
//      if(v == 0) return; //dont add 0 valued features to list of features
	...
	audit_data ad = {copy(base),feature_v.begin_,word_hash,v,true};
	ae->audit_features[index].push_back(ad);
      ...
      if ((affix_features[index] > 0) && (feature_name.end != feature_name.begin)) {
        ...
        while (affix > 0) {
          ...
          if (audit) {
            ...
            affix_v.push_back('\0');
            audit_data ad = {copy((char*)"affix"),affix_v.begin_,word_hash,v,true};
      ...
      if (spelling_features[index]) {
        ...
        substring spelling_ss = { spelling.begin_, spelling.end_};
        ...
        if (audit) {
          ...
          audit_data ad = {copy((char*)"spelling"),spelling_v.begin_,word_hash,v,true};
  ...
  inline void nameSpaceInfo(){
    if(reading_head == endLine ||*reading_head == '|' || *reading_head == ' ' || *reading_head == '\t' || *reading_head == ':' || *reading_head == '\r'){
      ...
      if(ae->atomics[index].begin_ == ae->atomics[index].end_)
	new_index = true;
  ...
  inline void nameSpace(){
    ...
    if(new_index && ae->atomics[index].begin_ != ae->atomics[index].end_)
