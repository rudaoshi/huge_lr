...
class beam {
 ...
 public:
  ...
  void do_recombination() {
    qsort(A.begin_, A.size(), sizeof(beam_element), compare_on_hash_then_cost);
    size_t start = 0;
  ...
  void compact(void (*free_data)(void*)=NULL) {
    ...
    qsort(A.begin_, A.size(), sizeof(beam_element), compare_on_cost); // TODO: quick select
    ...
    if (free_data)
      for (beam_element * be = A.begin_ +count; be != A.end_; ++be)
        free_data(be->data);

    A.end_ = A.begin_ + count;
  ...
  void erase(void (*free_data)(void*)=NULL) {
    if (free_data)
      for (beam_element * be = A.begin_; be != A.end_; ++be)
        free_data(be->data);
    A.erase();
    count = 0;
    worst_cost  = -FLT_MAX;
  ...
  beam_element * begin() { return A.begin_; }
  beam_element * end()   { return A.end_; }
