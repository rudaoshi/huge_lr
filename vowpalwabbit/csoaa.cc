...
namespace CSOAA {
  ...
  void predict_or_learn(csoaa& c, learner& base, example& ec) {
    ...
    ec.ld = &simple_temp;
    for (wclass *cl = ld->costs.begin_; cl != ld->costs.end_; cl ++)
      {
...
namespace LabelDict { 
  ...
  void del_example_namespace(example& ec, char ns, v_array<feature> features) {
    ...
    if (ec.atomics[(size_t)ns].size() == numf) { // did NOT have ns
      ...
      for (feature*f=features.begin_; f!=features.end_; f++) {
  ...
  void add_example_namespaces_from_example(example& target, example& source) {
    for (unsigned char* idx=source.indices.begin_; idx!=source.indices.end_; idx++) {
  ...
  void del_example_namespaces_from_example(example& target, example& source) {
    //for (size_t*idx=source.indices.begin_; idx!=source.indices.end; idx++) {
    unsigned char* idx = source.indices.end_;
    idx--;
    for (; idx>=source.indices.begin_; idx--) {
      if (*idx == constant_namespace) continue;
  ...
  void subtract_example(vw& all, example *ec, example *ecsub)
  {
    float norm_sq = 0.;
    size_t num_f = 0;
    for (unsigned char* i = ecsub->indices.begin_; i != ecsub->indices.end_; i++) {
      size_t feature_index = 0;
      for (feature *f = ecsub->atomics[*i].begin_; f != ecsub->atomics[*i].end_; f++) {
  ...
  void unsubtract_example(vw& all, example *ec)
  {
    ...
    if (all.audit) {
      if (ec->audit_features[wap_ldf_namespace].begin_ != ec->audit_features[wap_ldf_namespace].end_) {
        for (audit_data *f = ec->audit_features[wap_ldf_namespace].begin_; f != ec->audit_features[wap_ldf_namespace].end_; f++) {
          if (f->alloced) {
            free(f->space);
            free(f->feature);
            f->alloced = false;
  ...
  void do_actual_learning_wap(vw& all, ldf& l, learner& base, size_t start_K)
  {
    ...
    if (is_learn && !isTest) {
      for (size_t k=start_K; k<K; k++) {
        v_array<COST_SENSITIVE::wclass> this_costs = ((label*)l.ec_seq.begin_[k]->ld)->costs;
    ...
    for (size_t k1=start_K; k1<K; k1++) {
      example *ec1 = l.ec_seq.begin_[k1];
      ...
      for (size_t j1=0; j1<costs1.size(); j1++) {
        ...
        if (is_learn && !isTest) {
          ...
          for (size_t k2=k1+1; k2<K; k2++) {
            example *ec2 = l.ec_seq.begin_[k2];
  ...
  void do_actual_learning(vw& all, ldf& l, learner& base)
  {
    ...
    if (LabelDict::ec_seq_is_label_definition(l, l.ec_seq)) {  
      for (size_t i=0; i<l.ec_seq.size(); i++) {
        ...
        for (feature*f=l.ec_seq[i]->atomics[l.ec_seq[i]->indices[0]].begin_; f!=l.ec_seq[i]->atomics[l.ec_seq[i]->indices[0]].end_; f++) {
  ...
  void output_example_seq(vw& all, ldf& l)
  {
    if ((l.ec_seq.size() > 0) && !LabelDict::ec_seq_is_label_definition(l, l.ec_seq)) {
      ...
      bool hit_loss = false;
      for (example** ecc=l.ec_seq.begin_; ecc!=l.ec_seq.end_; ecc++)
